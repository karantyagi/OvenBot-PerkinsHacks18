...
def is_inside(curr,goal_ind,new_coords):
    ...
    if ((x - dx) * dax + (y - dy) * day > 0.0):
        return 0
    for each in new_coords[goal_ind]:
...
#####################################################################
            ...
            if freq == 0:
                ...
                speak.Speak("pop corn")
                cv2.imwrite("demo.png", cv2.flip(frame,1))
                break
            ...
            if freq == 1 and frames % 30 == 0:
                ...
                frequency = 2020
                winsound.Beep(2020, 6)
            elif freq == 3 and frames % 18 == 0:
                frequency = 2020
                winsound.Beep(2040, 6)
            elif freq == 4 and frames % 12 == 0:
                frequency = 2060
                winsound.Beep(2060, 6)
            elif freq == 5 and frames % 6 == 0:
                frequency = 2000
                winsound.Beep(2100, 6)
        ...
        key = cv2.waitKey(6) # milliseconds
        if frames%30 == 0:
